...
const FORM_STORAGE_KEY = 'hivy_surprise_booking_form_data';
...
export function FFCBookingForm({ pageTitle, variant = 'default', packageName, defaultPackageSlug, onClose }: FFCBookingFormProps) {
  ...
  const generateWhatsAppMessage = (data: FFCBookingFormData): string => {
    ...
    let message = `*New Booking Inquiry - HIVY Surprise Date*\n\n`;
  ...
  return (
    <Card className={`${variant === 'modal' ? 'border-0 shadow-none' : 'shadow-lg'} ${variant === 'hero' ? 'bg-white/60 backdrop-blur-md border-white/20' : ''} overflow-hidden`}>
      {variant !== 'modal' && (
        <div className={`${variant === 'hero' ? 'bg-gradient-to-r from-rose-700/95 to-pink-600/95' : 'bg-gradient-to-r from-rose-700 to-pink-600'} p-4 text-white`}>
      ...
      <CardContent className="p-4 sm:p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {selectedPkgSlug && (
                <div className="flex justify-end mt-1">
                  <button
                    type="button"
                    className="inline-flex items-center gap-1 text-[11px] sm:text-xs text-rose-800 hover:text-rose-900 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-full px-2.5 py-0.5 transition-colors font-medium cursor-pointer"
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      saveFormData(watchedValues);
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-rose-700 to-pink-600 hover:from-rose-800 hover:to-pink-700 text-white py-5 text-base font-semibold mt-2"
          >
            {isSubmitting ? (
              <>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button className={`bg-gradient-to-r from-rose-700 to-pink-600 hover:from-rose-800 hover:to-pink-700 text-white ${className}`}>
